...
Pass `architecture_plan.agents` + `domain_analysis` + workflows + routing patterns. Generate orchestrator and all subagents (parallel). Write to `~/.config/opencode/agent/`.
...
Pass command specifications + agent list + workflows. Each command specifies: target agent, description, syntax, examples, expected output. Write to `~/.config/opencode/command/`.
